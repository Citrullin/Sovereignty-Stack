...
use openidconnect::{
    ...
    ResponseTypes, Scope, StandardClaims, SubjectIdentifier, TokenUrl, UserInfoUrl, EndUserEmail, EndUserName,
};
...
async fn resolve_claims(
    ...
    let email = format!("{}@ens.eth", username);
    
    let mut name_localized = LocalizedClaim::new();
    name_localized.insert(None, EndUserName::new(username.clone()));
    
    StandardClaims::new(subject_id)
        .set_name(Some(name_localized))
...
pub async fn token(
    ...
    tracing::info!("token logic: requested code={}", form.code);
    let code_entry = if let Some(c) = db_client.get_code(form.code.to_string()).await? {
        tracing::info!("token logic: found code_entry for code={}", form.code);
        c
    } else {
        tracing::error!("token logic: Unknown code={}", form.code);
...
pub async fn sign_in(
    ...
    tracing::info!("sign_in: generating new code={}, client_id={}", code, params.client_id);
...
pub async fn userinfo(
    ...
    let code = if let Some(b) = bearer {
        ...
        tracing::error!("userinfo: Missing access token");
        return Err(CustomError::BadRequest("Missing access token.".to_string()));
    };
    tracing::info!("userinfo requested with access_token (code): {}", code);

    let code_entry = if let Some(c) = db_client.get_code(code.clone()).await? {
        c
    } else {
        tracing::error!("userinfo: Unknown code: {}", code);
        return Err(CustomError::BadRequest("Unknown code.".to_string()));
    };
    tracing::info!("userinfo: Found code_entry. Client ID: {}", code_entry.client_id);
