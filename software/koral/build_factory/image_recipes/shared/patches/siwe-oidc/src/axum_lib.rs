...
async fn userinfo(
    ...
    Extension(private_key): Extension<RsaPrivateKey>,
    Extension(config): Extension<config::Config>,
    query: Option<Query<oidc::UserInfoPayload>>,
    ...
    let extracted_token = bearer.map(|b| b.0 .0.token().to_string())
        .or_else(|| {
    ...
    let mut payload = if let Some(Form(p)) = payload {
        p
    } else if let Some(Query(q)) = query {
        q
    } else {
    ...
    if payload.access_token.is_none() {
        payload.access_token = extracted_token;
    }
