...
function authenticate_with_frappe(socket, next) {
	...
	const site_name = get_site_name(socket);

	// Bypass Origin Validation behind Istio
	// if (origin && origin !== host && origin !== site_name && origin !== "erpnext.homelab.local" && conf.allow_cors !== "*" && get_hostname(conf.allow_cors) !== origin) {
	...
	socket.frappe_request = (path, args = {}, opts = {}) => {
		...
		// FIXED: Use internal service name instead of fragile get_url
		const backend_url = "http://erpnext-gunicorn:8000";
