...
def get_oauth2_authorize_url(provider: str, redirect_to: str) -> str:
	...
	data = {"redirect_uri": get_redirect_uri(provider), "state": base64.b64encode(json.dumps(state).encode("utf-8"))}

	if auth_url_data := get_oauth2_providers()[provider].get("auth_url_data"):
		data.update(auth_url_data)

	return flow.get_authorize_url(**data)


def get_redirect_uri(provider: str):
	return frappe.utils.get_url(get_oauth2_providers()[provider]["redirect_uri"])


def get_oauth2_flow(provider: str) -> OAuth2Service:
	params = get_oauth_keys(provider)
	params.update(get_oauth2_providers()[provider]["flow_params"])
...
def get_info_via_oauth(provider: str, code: str, decoder: Callable | None = None, id_token: bool = False):
	...
	if id_token:
		...
		info = session.get(api_endpoint, params=api_endpoint_args, verify=(provider != 'authentik')).json()
	...
	if provider == 'authentik':
		pass
	elif not (info.get("email_verified") or info.get("email")):
...
def login_oauth_user(
	...
	*,
	...
	user = get_email(data).lower() if get_email(data) else None
...
def get_user_record(user: str, data: dict, provider: str) -> "User":
	...
	user_doc: User = frappe.new_doc("User")
...
def update_oauth_user(user: str, data: dict, provider: str):
	...
	user_doc: User = get_user_record(user, data, provider)
	update_user_record = user_doc.is_new()

	if not user_doc.enabled:
		frappe.respond_as_web_page(_("Not Allowed"), _("User {0} is disabled").format(user_doc.email))
		return False

	if not user_doc.get_social_login_userid(provider):
		update_user_record = True
		match provider:
			case "facebook":
				user_doc.set_social_login_userid(provider, userid=data["id"], username=data.get("username"))
				user_doc.update({"user_image": f"https://graph.facebook.com/{data['id']}/picture"})
			case "google":
				user_doc.set_social_login_userid(provider, userid=data["id"])
			case "github":
				user_doc.set_social_login_userid(provider, userid=data["id"], username=data.get("login"))
			case "frappe" | "office_365":
				user_doc.set_social_login_userid(provider, userid=data["sub"])
			case "salesforce":
				user_doc.set_social_login_userid(provider, userid="/".join(data["sub"].split("/")[-2:]))
			case _:
				user_id_property = (
					frappe.db.get_value("Social Login Key", provider, "user_id_property") or "sub"
				)
				user_doc.set_social_login_userid(provider, userid=data[user_id_property])
